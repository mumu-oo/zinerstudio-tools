...
const APP_TITLE = 'COIN LEDGER GB-style';
const DATA_FILE_NAME = 'coin-ledger-data.json';
...
ipcMain.handle('coin-ledger:load-data', async () => {
  return await readLedgerData();
});

ipcMain.handle('coin-ledger:save-data', async (event, payload) => {
  return await writeLedgerData(payload);
});

ipcMain.handle('coin-ledger:export-backup', async (event, options = {}) => {
  const win = BrowserWindow.fromWebContents(event.sender);
  if (!win) return { ok: false, canceled: true };

  const suggestedName = options.filename || 'COIN_LEDGER_GB-style_backup.json';
  const { canceled, filePath } = await dialog.showSaveDialog(win, {
    title: '匯出 COIN LEDGER 備份',
...
ipcMain.handle('coin-ledger:import-backup', async (event) => {
  const win = BrowserWindow.fromWebContents(event.sender);
  if (!win) return { ok: false, canceled: true };

  const { canceled, filePaths } = await dialog.showOpenDialog(win, {
    title: '讀取 COIN LEDGER 備份',
    properties: ['openFile'],
    filters: [{ name: 'JSON', extensions: ['json'] }]
  });
...
ipcMain.handle('coin-ledger:reveal-data-file', async () => {
  const filePath = getDataFilePath();
  await fs.mkdir(path.dirname(filePath), { recursive: true });
      ...
      submenu: [
        {
          ...
          click: () => sendToFocused('coin-ledger:menu-export-backup')
        },
        {
          label: '讀取備份',
          accelerator: 'CmdOrCtrl+O',
          click: () => sendToFocused('coin-ledger:menu-import-backup')
        },
        { type: 'separator' },
        {
          label: '顯示資料檔位置',
          accelerator: 'CmdOrCtrl+Shift+O',
          click: () => sendToFocused('coin-ledger:menu-reveal-data-file')
        }
      ]
    }
  ];
